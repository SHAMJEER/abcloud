/*!  AB 3 - ab file manager main functions */
/* 
 * Close alert message
 */
function closeAlert(){
...
function checkNextPrev(currentID){
    ...
    var nextgall = current.firstAfter('.gallindex').find('. AB-gall');
    var prevgall = current.firstBefore('.gallindex').find('. AB-gall');
    ...
    if (nextgall.length > 0){
        ...
        if ($('.nextgall').length < 1) {
            $('. AB-zoom').append('<a class="nextgall navigall"><span class="fa-stack"><i class="fa fa-angle-right fa-stack-1x fa-inverse"></i></span></a>');
        }
    ...
    if (prevgall.length > 0){
        ...
        if ($('.prevgall').length < 1) {
            $('. AB-zoom').append('<a class="prevgall navigall"><span class="fa-stack"><i class="fa fa-angle-left fa-stack-1x fa-inverse"></i></span></a>');
        }
...
$(document).on('submit', '#sendpwd', function(event) {
    ...
    $('.mailpreload').fadeIn(function(){
        $.ajax({
            type: "POST",
            url: "abcloud-admin/ajax/sendpwd.php",
            data: serialize
            })
            .done(function( msg ) {
                $('.sendresponse').html(msg).fadeIn();
                $('.mailpreload').fadeOut();
                $('#captcha').attr('src', 'abcloud-admin/captcha/img.php?' + time);
...
function updateSession($data){
    $.ajax({
        method: "POST",
        url: "abcloud-admin/ajax/session.php",
        data: $data,
...
// lengthMenu : [[5, 10, 25, 50, 100, 200], [5, 10, 25, 50, 100, 200]],
    ...
    */
    ...
    fileTable = $('#filetable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url : "abcloud-admin/ajax/get-files.php",
            // type: 'POST',
            data : {
        ...
        drawCallback: function( settings ) {
            ...
            // Stop all sounds
            if ($('.sm2_button').length &&  ABinlinePlayer !== null) {
                 ABinlinePlayer.removeEventHandler(document, 'click',  ABinlinePlayer.handleClick);
                 ABinlinePlayer.init();
            }
        },
        ...
        initComplete : function() {
            ...
            $(this).closest('. ABblock').find('.overload').remove();
    ...
    */
    ...
    folderTable = $('#foldertable').DataTable( {
        ...
        ajax: {
            url : "abcloud-admin/ajax/get-dirs.php",
            // type: 'POST',
            data : {
                'dir' : tablesettings.dir,
                // 'fullpath' : tablesettings.fullpath
        ...
        if (!$(this).hasClass('disabled')) {
            ...
            if (search.length) {
                ...
                $.ajax({
                    ...
                    type: "GET",
                    dataType: 'json',
                    url: "abcloud-admin/ajax/get-search.php?s=" + search
                })
                .done(function( msg ) {
...
function createZipDir(folder, dash){
    ...
    $.ajax({
        cache: false,
        type: "POST",
        url: "abcloud-admin/ajax/zip.php?t=" + (new Date).getTime(),
        data: {
            fold: folder,
            dash: dash
        }
    ...
    */
    $(document).on('click', '#createlink', function() {
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url: "abcloud-admin/ajax/shorten.php",
            data: {
                atts: divar.join(','),
                time: $time,
                hash: $hash,
        ...
        if (numfiles > 0) { 
            ...
            $('#sendfiles').unbind('submit').submit(function(event) {
                ...
                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "abcloud-admin/ajax/sendfiles.php?t=" + now,
        ...
        bootbox.confirm({
            ...
            callback: function(answer) {
                ...
                if (answer) {
                    $.ajax({
                        type: "POST",
                        url: "abcloud-admin/ajax/ AB-del.php",
                        data: deldata
                    })
                    .done(function( msg ) {
                        if (msg == 'ok') {
                    ...
                    .fail(function() {
                        $('.delresp').html( 'error connecting to  AB-del.php' );
                    });
                }
            }
        });
...
function setupFolderTree(currentdir, __root) {
    ...
    $('.archive-map').on('show.bs.modal', function (event) {
        ...
        if (!wrapresult.hasClass('loaded')) {
            ...
            var request = $.ajax({
                url: "abcloud-admin/ajax/get-filetree.php",
                method: "POST",
                data: { 
                    'action' : action,
...
function setupMove($activepagination, $selectfiles, $time, $hash) {
    ...
    $(document).on('click', '.movelink', function(e) {
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url: "abcloud-admin/ajax/ AB-move.php?tm=" + (new Date).getTime(),
            data: movedata
        })
        .done(function(msg) {
            if (msg == 'ok') {
        ...
        .fail(function() {
            var alert = '<div class="alert alert-danger" role="alert">Error connecting  AB-move.php</div>'
            $hiddenalert.html(alert);
        });
    });
}
