...
if ($gateKeeper->isAccessAllowed() && $location->editAllowed('../../') && $gateKeeper->isAllowed('view_enable')) {
    ...
    include_once '../fonts/ AB-icons.php';
    ...
    if (count($getfiles) > 0) {
        ...
        foreach ($getfiles as $key => $file) {
            ...
            if ($totcounter > $start) {
                ...
                $data = array();
                $data['DT_RowId'] = ' ABRow-'.$totcounter;
                ...
                if ($setUp->getConfig('enable_prettylinks') == true) {
                    ...
                    $downlink = 'abcloud-admin/ AB-downloader.php?q='.$thislink.'&h='.$dash;
                ...
                if ($file->isValidForAudio()) {
                    ...
                    if ($setUp->getConfig('enable_prettylinks') == true) {
                        ...
                        $linkaudio = "abcloud-admin/ AB-downloader.php?q=".$thislink."&h=".$dash;
                    }
                    if ($gateKeeper->isAllowed('download_enable')) {
                        $data['icon'] .= '<a type="audio/mp3" id=" AB-audio-'.$key.'" class="sm2_button" href="'.$linkaudio.'&audio=play">';
