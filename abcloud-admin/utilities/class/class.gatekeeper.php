<?php
    ...
    class GateKeeper
    {
         ...
         * @return $message
         */
        public static function init($relative = 'abcloud-admin/', $callfrom = '')
        {
            global $gateKeeper;
            global $cookies;
            global $updater;
            ...
            if (isset($_GET['logout'])) {
                $unsetuser = isset($_SESSION[' AB_user_name']) ? $_SESSION[' AB_user_name'] : false;
                $_SESSION[' AB_user_name'] = null;
                $_SESSION[' AB_logged_in'] = null;
                $_SESSION[' AB_user_name_new'] = null;
                $_SESSION[' AB_user_space'] = null;
                $_SESSION[' AB_user_used'] = null;
            ...
            $rememberme = filter_input(INPUT_POST, ' AB_remember', FILTER_SANITIZE_STRING);
            ...
            if ($postusername && $postuserpass) {
                ...
                if (Utils::checkCaptcha('show_captcha'.$callfrom)) {
                    ...
                    if (GateKeeper::isUser($postusername, $postuserpass)) {
                        ...
                        if (isset($_SESSION[' AB_user_name_new']) && strlen($_SESSION[' AB_user_name_new']) > 0) {
                            $postusername = $_SESSION[' AB_user_name_new'];
                            // unset old sensitive username
                            $updater->updateUserData($postusername, 'sensitive', false);
                            $updater->updateUsers('abcloud-admin/users/');
                        }

                        $_SESSION[' AB_user_name'] = $postusername;
                        $_SESSION[' AB_logged_in'] = 1;

                        $usedspace = $gateKeeper->getUserSpace();

                        if ($usedspace !== false) {
                            $userspace = $gateKeeper->getUserInfo('quota')*1024*1024;
                            $_SESSION[' AB_user_used'] = $usedspace;
                            $_SESSION[' AB_user_space'] = $userspace;
                        } else {
                            $_SESSION[' AB_user_used'] = null;
                            $_SESSION[' AB_user_space'] = null;
                        }
            ...
            foreach ($users as $user) {
                if (isset($user['sensitive']) && $user['sensitive'] === $userName) {
                    if (crypt($passo, $user['pass']) == $user['pass']) {
                        $_SESSION[' AB_user_name_new'] = $user['name'];
        ...
        {
            if (isset($_SESSION[' AB_user_name'])
                && isset($_SESSION[' AB_logged_in'])
                && $_SESSION[' AB_logged_in'] == 1
            ) {
                return true;
            }
        ...
        public static function getUserInfo($info)
        {
            if (GateKeeper::isUserLoggedIn()
                && isset($_SESSION[' AB_user_name'])
                && strlen($_SESSION[' AB_user_name']) > 0
            ) {
                $username = $_SESSION[' AB_user_name'];
         ...
         * @return image path
         */
        public static function getAvatar($username, $adminarea = 'abcloud-admin/', $size = '25')
        {
            ...
            if (!file_exists($adminarea.'_content/avatars/'.$avaimg)) {
                ...
                $imgtag = '<img class="img-circle avatar" width="'.$size.'" src="'.SetUp::getConfig('script_url').'abcloud-admin/_content/avatars/'.$avaimg.'">';
            }
            return $imgtag;
        }
        ...
        public function setCookie($postusername = false)
        {
            ...
            if (PHP_VERSION_ID >= 70300) {
                ...
                setcookie(
                    ' AB_user_name', $postusername,
                    ['expires' => $expires, 'httponly' => true]
                    // ['expires' => $expires, 'httponly' => true, 'samesite' => 'Strict']
                );
            } else {
                setcookie('rm', $rmsha, $expires);
                setcookie(' AB_user_name', $postusername, $expires);
            }
            ...
            if (!array_key_exists($postusername, $_REMEMBER)
                ...
                if (false == (file_put_contents(
                    'abcloud-admin/users/remember.php',
         ...
         */
        public static function removeCookie($postusername = false, $path = 'abcloud-admin/')
        {
            global $_REMEMBER;
        ...
        public function checkCookie()
        {
            ...
            if (isset($_COOKIE['rm']) && isset($_COOKIE[' AB_user_name'])) {
                $name = $_COOKIE[' AB_user_name'];
                $key = $_COOKIE['rm'];
