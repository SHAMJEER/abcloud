<?php
    ...
    class Uploader
    {
        ...
        public static function veryFile($fullfilepath, $filesize, $remote = false)
        {
            ...
            if (!$extension) {
                Utils::setError(
                    '<span><i class="fa fa-exclamation-triangle"></i> <strong>'.$basename.'</strong>: '
                    .SetUp::getLangString('file_extension_missing').'</span> '
                );
                return false;
            }
            ...
            if ((!$allowed_ext && !$rejected_ext)
                ...
                Utils::setError(
                    '<span><i class="fa fa-exclamation-triangle"></i> '.$filename.'<strong>.'.$extension.'</strong> '
                    .SetUp::getLangString('not_allowed').'</span> '
                );
                return false;
            }
            ...
            if (Utils::fileExists($fullfilepath)) {
                Utils::setWarning(
                    '<span><i class="fa fa-info-circle"></i> <strong>'.$basename.'</strong> '
                    .SetUp::getLangString('file_exists').'</span> '
                );
                return false;
            }
            ...
            if (Actions::checkUserSpace(false, $filesize, true) === false) {
                ...
                Utils::setError(
                    '<span><i class="fa fa-exclamation-triangle"></i> '.SetUp::getLangString('upload_exceeded').': <strong>'.$max_upload_response.'</strong></span> '
                );
                return false;
            }
            return true;
        ...
        //  Prepend date-time before file name.
        ...
        public function createFileFromChunks($destination, $temp_dir, $fileName, $totalSize, $totalChunks, $logloc)
        {
            ...
            if (is_dir($upload_dir) && ($openfile = fopen($upload_dir.$finalfile, 'w')) !== false) {
                ...
                Utils::setSuccess(' <span><i class="fa fa-check-circle"></i> <strong>'.$finalfile.'</strong></span> ');
                ...
                Logger::log($message, '');
                // Deprecated: new log notification for multiple uploads called @ uploaders.js
                // if (SetUp::getConfig('notify_upload')) {
                //     Logger::emailNotification($logloc.$finalfile, 'upload');
                // }
            } else {
                Utils::setError(' <span><i class="fa fa-exclamation-triangle"></i> cannot create the destination file inside <strong>'.$upload_dir.'</strong></span>');
                return false;
            }
        }
        ...
        public function logUpload($log_start_time, $logmethod, $totalSize)
        {
            ...
            $log .= '********************************************'.PHP_EOL.PHP_EOL;
            file_put_contents(' AB_uploader_log.txt', $log, FILE_APPEND);
        }
        ...
        public function getRemoteSize($url)
        {
            ...
            if (filter_var($url, FILTER_VALIDATE_URL)) {
                ...
                if (curl_error($cinit)) {
                    ...
                    if ($htmltype !== false) {
                        Utils::setError(' <span><i class="fa fa-exclamation-triangle"></i> '.SetUp::getLangString('invalid_link').'</span>');
                        $avoid = true;
                    }
                    if (!$avoid) {
        ...
        public static function uploadFile($location, $thename, $tempname, $size)
        {
            ...
            if (!$location->editAllowed() || !$location->isWritable()) {
                Utils::setError('<span><i class="fa fa-exclamation-triangle"></i> '.$setUp->getString('upload_not_allowed').'</span> ');
                return false;
            }
            ...
            if (Uploader::veryFile($upload_file, $size) == true) {
                ...
                if (!is_uploaded_file($tempname)) {
                    ...
                    Utils::setSuccess('<span><i class="fa fa-check-circle"></i> <strong>'.$name.'</strong></span> ');
